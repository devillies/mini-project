...
class App extends Component<{}, State> {
  ...
  addValue = (_event: SyntheticEvent<Target>) => {
    let newValue = _event.target.value;
    this.setState({ newList: newValue });
  };
  addTodo = (_event: SyntheticEvent<Target>) => {
  ...
  rmTodo = (_event: SyntheticEvent<T>) => {
    this.setState({
  ...
  render() {
    ...
    let filteredList = todoList.filter(item => item.name.includes(searchValue));
    return (
      <div>
        <input
          value={searchValue}
